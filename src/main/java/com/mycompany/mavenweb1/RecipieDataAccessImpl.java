...
//            }
            ...
            for (Recipe rec : recipies) {
                ...
                if (em.contains(r)) {
                    ...
                    r.setImagepath(rec.getImagepath());
                    r.setName(rec.getName());
                    r.setPrice(rec.getPrice());
                    System.out.println("recipe updated" + rec.getName());
                } else {
                    em.persist(rec);
                    System.out.println("recipe stored" + rec.getName());
                }
            }
        }
    ...
    public List<Cartdb> getCarts() {
        ...
        if (em != null) {
            //   use this instead of the simple query if you know what type to expect
            //   TypedQuery<Cartdb> q2 = em.createQuery("select cartdb from Cartdb cartdb",Cartdb.class);
    ...
    /**
     * Метод вызывается при сохранении изменённого списка заказов
     * администратором. Заказы удаляются из БД, если они выполнены
     *
